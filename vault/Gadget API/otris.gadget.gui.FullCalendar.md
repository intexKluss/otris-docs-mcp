---
title: "Class: FullCalendar"
source: "https://otris.software/documents/api/gadgets/otris.gadget.gui.FullCalendar.html"
---
...
| Name | Type | Description |
| --- | --- | --- |
| soure | FullCalendarSource | A fullcalendar source definition |
| eventLoader | function | optional eventLoader function for this source. See details setEventLoader |
...
| Name | Type | Description |
| --- | --- | --- |
| calConfig | object | optional calendar config. See fullcalendar documentation for details |
...
| Name | Type | Description |
| --- | --- | --- |
| doubleClickFunction | function | The given function is executed if the caleandar is double clicked function(Object event, String dateString, DocumentsContext documentsContext) doubleClickFunction(event, dateString, documentsContext) Double click function Name Type Description event object Event dateString string The date as string of the clicked item documentsContext documents.sdk.DocumentsContext The DocumentsContext instance | Name | Type | Description | event | object | Event | dateString | string | The date as string of the clicked item | documentsContext | documents.sdk.DocumentsContext | The DocumentsContext instance |
| Name | Type | Description |
| event | object | Event |
| dateString | string | The date as string of the clicked item |
| documentsContext | documents.sdk.DocumentsContext | The DocumentsContext instance |
...
return the events for the given source in the specified time range `start - end`

The options object can contain a filter string (`options.filter`) if the search context is used (see [[Gadget API/otris.gadget.gui.FullCalendar#setSearchContext|setSearchContext]])

Required return format: `Array of FullCalendarEvents`

| Name | Type | Description |
| --- | --- | --- |
| Function | function | with the following signature: function eventLoader(FullCalendarSource eventLoaderFunction source, Date start, Date end,(String|Boolean) timezone, (Object) options) |
...
| Name | Type | Default | Description |
| --- | --- | --- | --- |
| lazyLoad | boolean | true | optional Lazy load the events? |
...
The `filterFunction` parameter is only required if `lazyLoad` is set to false.

| Name | Type | Description |
| --- | --- | --- |
| label | string | Label for the search box |
| filterFunction | function | optional [optional since 6.1.2] Function with the following signature: function filterFunction(source, searchExpression) - For FullCalendar v6.x: source is the plain event array - For FullCalendar v3.x: source is the event source object (.eventDefs or .events) The function must return the filtered event array. |
...
```
// FullCalendar v6.x (since documentsOS 6.1.2) – filterFunction optional
gadgetCalendar.setSearchContext("Gadget_Calendar"); // uses defaultFilterFunction

gadgetCalendar.setSearchContext("Gadget_Calendar", function filterFunction(events, searchExpression) {
  return events.filter(e => e.title.toLowerCase().includes(searchExpression));
});
```


```
// FullCalendar v3.10.x in DOCUMENTS 5
calendarGadget.setSearchContext("Gadget_Calendar", function filterFunction(source, searchExpression) {
  // filter events containing searchExpression
  return source.eventDefs.filter(e => e.title.toLowerCase().includes(searchExpression));
});
```
...
calendarGadget.setSearchContext("Gadget_Calendar", function filterFunction(source, searchExpression) {
  // filter events containing searchExpression
  return source.events.filter(e => e.title.toLowerCase().includes(searchExpression));
});
```
