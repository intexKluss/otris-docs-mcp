---
title: "Class: ScriptListWrapper"
source: "https://otris.software/documents/api/gadgets/otris.gadget.gui.ScriptListWrapper.html"
---
...
| --- | --- | --- |
| scriptList | otris.scriptlist.List | ScriptList to be wrapped |
...
```
// ScriptListWrapper with auto height
gadgetAPI.registerGadgetAction("gadgetField_autoHeight", () => {

 // create wrapper & list
 const wrapper = gadgetAPI.getScriptListWrapperInstance();
 const list = new scriptListAPI.List("gadgetField_autoHeight");
 wrapper.addScriptList(list);

 // set options
 list.setAutoHeight(true); // use auto height
 list.endList();
 list.setShowToolbar(false);

 // add column & row
 list.addColumn("column1", "Spalte 1");
 list.addRow("0", { column1: "Zeile 1" });
...
```
// ScriptListWrapper with fixed height
gadgetAPI.registerGadgetAction("gadgetField_fixedHeight", () => {
  ...
  // set options
  wrapper.setStyle("height", 100); // set fixed height
  list.endList();
  list.setShowToolbar(false);
